  ...
  const handleCategoryClick = async (category) => {
    ...
    try {
        ...
        if (filteredCourses.length > 0) {
            setCourse(filteredCourses[0]); // Set the first course as the default selected course
        }
    } catch (error) {
...
const handleFinishQuiz = () => {
  ...
  const handleCommentSubmit = async (courseId, apprenantId) => {
    try {
        ...
        if (!commentValue) {
            throw new Error('Aucun commentaire trouvé pour ce cours');
        }

        // Effectuer une requête API pour récupérer les détails de l'apprenant
        const apprenantResponse = await axios.get(`http://localhost:3001/api/apprenants/${apprenantId}`);
        const apprenant = apprenantResponse.data;
        if (!apprenant || !apprenant.nom) {
            throw new Error('Impossible de récupérer les détails de l\'apprenant');
        }
        const nomApprenant = apprenant.nom;

        // Effectuer une requête API pour ajouter le commentaire au cours avec le nom de l'apprenant
        const response = await axios.post(`http://localhost:3001/api/${courseId}/commentaires`, {
            commentaire: commentValue,
            apprenantId: apprenantId,
            nomApprenant: nomApprenant // Ajouter le nom de l'apprenant
        });
        ...
        setCourseComments(prevComments => ({
            ...
            [courseId]: [
                ...
                { commentaire: commentValue, nomApprenant: nomApprenant } // Ajouter le nouveau commentaire avec le nom de l'apprenant
            ]
        }));
